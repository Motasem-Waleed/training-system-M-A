...
export default function AddPsychologist() {
  ...
  const [form, setForm] = useState({
    ...
    major: "", department_id: "", role_id: 6, status: "active",
  });
  const isEditMode = !!id;
    ...
    if (id) {
      const fetchUser = async () => {
        try {
          ...
          setForm({ name: userData.name || "", email: userData.email || "", password: "", password_confirmation: "", major: userData.major || "", department_id: userData.department_id || "", role_id: userData.role_id || 6, status: userData.status || "active" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) { await updateUser(id, form); setStatusMessage({ type: "success", text: "تم تحديث الأخصائي النفسي بنجاح" }); }
      else { await createUser(form); setStatusMessage({ type: "success", text: "تمت إضافة الأخصائي النفسي بنجاح" }); setForm({ name: "", email: "", password: "", password_confirmation: "", major: "", department_id: "", role_id: 6, status: "active" }); }
  ...
  return (
    <div className="user-form">
      <div className="page-header"><h1>{isEditMode ? "تعديل أخصائي نفسي" : "إضافة أخصائي نفسي جديد"}</h1><button onClick={() => navigate("/admin/users")} className="btn-secondary">رجوع</button></div>
