...
export default function AddAcademicSupervisor() {
  ...
  const [form, setForm] = useState({
    ...
    major: "", department_id: "", role_id: 7, status: "active",
  });
  const isEditMode = !!id;
    ...
    if (id) {
      const fetchUser = async () => {
        try {
          ...
          setForm({ name: userData.name || "", email: userData.email || "", password: "", password_confirmation: "", major: userData.major || "", department_id: userData.department_id || "", role_id: userData.role_id || 7, status: userData.status || "active" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) { await updateUser(id, form); setStatusMessage({ type: "success", text: "تم تحديث المشرف الأكاديمي بنجاح" }); }
      else { await createUser(form); setStatusMessage({ type: "success", text: "تمت إضافة المشرف الأكاديمي بنجاح" }); setForm({ name: "", email: "", password: "", password_confirmation: "", major: "", department_id: "", role_id: 7, status: "active" }); }
  ...
  return (
    <div className="user-form">
      <div className="page-header"><h1>{isEditMode ? "تعديل مشرف أكاديمي" : "إضافة مشرف أكاديمي جديد"}</h1><button onClick={() => navigate("/admin/users")} className="btn-secondary">رجوع</button></div>
      ...
      <form onSubmit={handleSubmit} className="form">
        ...
        <div className="form-group"><label>القسم *</label><select name="department_id" value={form.department_id} onChange={handleChange} required><option value="">اختر القسم</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select>{errors.department_id && <span className="error">{errors.department_id[0]}</span>}</div>
